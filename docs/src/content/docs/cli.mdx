...
After scaffolding, `init` automatically installs backend dependencies (`@zeroback/server`) using the detected package manager.
...
<Tabs>
  ...
  <TabItem label="bun">
    ```bash
    bunx @zeroback/cli dev
    # or with a custom functions directory
    bunx @zeroback/cli dev ./src/zeroback
    ```
  </TabItem>
  <TabItem label="pnpm">
    ```bash
    pnpx @zeroback/cli dev
    # or with a custom functions directory
    pnpx @zeroback/cli dev ./src/zeroback
    ```
  </TabItem>
  <TabItem label="yarn">
    ```bash
...
<Tabs>
  ...
  <TabItem label="bun">
    ...
    # Deploy
    bunx @zeroback/cli deploy

    # Dry run (codegen only)
    bunx @zeroback/cli deploy --dry-run

    # Pass args to wrangler
    bunx @zeroback/cli deploy -- --env production

    # CI/CD: use an API token instead of interactive login
    CLOUDFLARE_API_TOKEN=your-token bunx @zeroback/cli deploy
    ```
  </TabItem>
  <TabItem label="pnpm">
    ```bash
    ...
    # Deploy
    pnpx @zeroback/cli deploy

    # Dry run (codegen only)
    pnpx @zeroback/cli deploy --dry-run

    # Pass args to wrangler
    pnpx @zeroback/cli deploy -- --env production

    # CI/CD: use an API token instead of interactive login
    CLOUDFLARE_API_TOKEN=your-token pnpx @zeroback/cli deploy
    ```
  </TabItem>
  <TabItem label="yarn">
    ```bash
...
<Tabs>
  ...
  <TabItem label="bun">
    ```bash
    bunx @zeroback/cli codegen
    ```
  </TabItem>
  <TabItem label="pnpm">
    ```bash
    pnpx @zeroback/cli codegen
    ```
  </TabItem>
  <TabItem label="yarn">
    ```bash
...
<Tabs>
  ...
  <TabItem label="bun">
    ```bash
    bunx @zeroback/cli reset
    ```
  </TabItem>
  <TabItem label="pnpm">
    ```bash
    pnpx @zeroback/cli reset
    ```
  </TabItem>
  <TabItem label="yarn">
    ```bash
...
<Tabs>
  ...
  <TabItem label="bun">
    ```bash
    # Run a query
    bunx @zeroback/cli run tasks:list

    # Run a mutation with arguments
    bunx @zeroback/cli run tasks:create '{"title": "Buy groceries", "projectId": "proj:abc", "status": "todo"}'

    # Run an internal function
    bunx @zeroback/cli run tasks:countInternal '{"projectId": "proj:abc"}'

    # Target a deployed server
    bunx @zeroback/cli run tasks:list --url https://my-worker.example.com
    ```
  </TabItem>
  <TabItem label="pnpm">
    ```bash
    # Run a query
    pnpx @zeroback/cli run tasks:list

    # Run a mutation with arguments
    pnpx @zeroback/cli run tasks:create '{"title": "Buy groceries", "projectId": "proj:abc", "status": "todo"}'

    # Run an internal function
    pnpx @zeroback/cli run tasks:countInternal '{"projectId": "proj:abc"}'

    # Target a deployed server
    pnpx @zeroback/cli run tasks:list --url https://my-worker.example.com
    ```
  </TabItem>
  <TabItem label="yarn">
    ```bash
