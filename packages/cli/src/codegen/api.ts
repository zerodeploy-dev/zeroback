...
export function generateApi(manifest: FunctionManifest, outputPath: string): void {
  ...
  const lines: string[] = [
    ...
    ``,
    `// Extracts typed FunctionReference from a registered function`,
    `type FnRef<T> = T extends { _type: infer Type extends "query" | "mutation" | "action"; _args: infer Args; _returns: infer Returns }`,
    `  ? { _name: string; _type: Type; _isInternal: boolean; _args: Args; _returns: Returns }`,
    `  : never`,
  ];
