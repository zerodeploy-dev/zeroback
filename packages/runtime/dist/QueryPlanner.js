...
export function queryTable(sql, schemaInfo, tableColumns, table, asOfTs, filter, indexQuery, orderField, orderDirection, limit, keysetCursor, searchQuery) {
    ...
    if (isSearch) {
        ...
        if (tableSchema?.searchIndexes) {
            const si = tableSchema.searchIndexes.find((s) => s.searchField === searchQuery.searchField);
            if (si) {
                resolvedFtsTable = `${table}_${si.name}`;
            }
        }
    }
    const conditions = [`${colPrefix}_ts <= ?`];
    const params = [asOfTs];
    // FTS MATCH condition
 ...
 * e.g. `"status" = ?` → `m."status" = ?`
 */
function prefixFilterColumns(sql, alias) {
