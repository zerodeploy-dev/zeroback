 ...
 */
function extractTenant(pathname) {
...
async function handleStorageUpload(request, env, slug, baseUrl, url) {
    const r2 = env.ZEROBACK_STORAGE;
    if (!r2) {
        return new Response(JSON.stringify({ error: "File storage not configured. Add a [[r2_buckets]] binding named ZEROBACK_STORAGE to your wrangler.toml." }), { status: 500, headers: { "Content-Type": "application/json" } });
    }
    ...
    if (!validateRes.ok) {
        ...
        return new Response(JSON.stringify({ error: body.error ?? "Invalid upload token" }), { status: 403, headers: { "Content-Type": "application/json" } });
    }
    const { doId: doIdStr } = await validateRes.json();
...
async function handleStorageDownload(env, slug, storageId) {
    const r2 = env.ZEROBACK_STORAGE;
    if (!r2) {
        return new Response(JSON.stringify({ error: "File storage not configured" }), { status: 500, headers: { "Content-Type": "application/json" } });
    }
