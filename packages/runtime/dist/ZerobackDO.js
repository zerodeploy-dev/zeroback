...
export function createZerobackDO(config) {
    return class ZerobackDO extends DurableObject {
        latestTs = 0;
        transactions;
        subscriptions;
        connections;
        ...
        restoreConnectionsFromHibernation() {
    ...
    `);
        ...
        restoreBaseUrl() {
            this.ctx.storage.get("baseUrl").then((url) => {
                if (url)
                    this.storage.setBaseUrl(url);
            });
        }
        async loadLatestTs() {
            const stored = await this.ctx.storage.get("latestTs");
        ...
        async fetch(req) {
            ...
            if (path === "/__internal/validate-upload" && req.method === "POST")
                return this.storage.handleValidateUpload(url);
            if (path === "/__internal/storage-record" && req.method === "POST")
                return this.storage.handleStorageRecord(req);
            if (path === "/__internal/storage-delete" && req.method === "POST")
                return this.storage.handleStorageDelete(req);
        ...
        async handleHttpAction(handler, req) {
            try {
        ...
        async handleAdminRun(req) {
            ...
            try {
                ...
                if (!fn) {
                    return new Response(JSON.stringify({ success: false, error: `Function not found: ${fnName}`, code: ErrorCode.NOT_FOUND }), { status: 404, headers: json });
                }
                let result;
                if (fn.type === "mutation") {
                ...
                return new Response(JSON.stringify({ success: true, result }), { status: 200, headers: json });
            }
            catch (e) {
                return new Response(JSON.stringify({ success: false, error: errorMessage(e), code: ErrorCode.EXECUTION_ERROR }), { status: 500, headers: json });
            }
        }
        // -- WebSocket --
        ...
        createDbOps(txId) {
            return {
                query: async (table, filter, orderField, orderDirection, limit, indexQuery, keysetCursor, searchQuery) => {
                    ...
                    if (searchQuery) {
                        ...
                        descriptorFilter = filter && indexFilter
                            ? { op: "and", exprs: [indexFilter, filter] }
                            : (indexFilter || filter);
                    }
                    this.transactions.addQueryDescriptor(txId, { table, filter: descriptorFilter });
                    const docs = queryTable(this.sql, this.schemaInfo, this.tableColumns, table, tx.beginTs, filter, indexQuery ?? null, orderField, orderDirection, limit, keysetCursor, searchQuery ?? null);
                ...
                insert: async (table, id, data) => {
                    this.validateDocument(table, data);
        ...
        // -- Function execution --
        async invokeFunction(fnName, args, txId) {
            const fn = this.functions[fnName];
            if (!fn) {
            ...
            if (fn.argsValidator && Object.keys(fn.argsValidator).length > 0) {
                const argsSchema = {
                    type: "object",
                    value: Object.fromEntries(Object.entries(fn.argsValidator).map(([k, v]) => [k, v.json])),
                };
        ...
        // -- WebSocket handlers --
        async handleQuery(connectionId, msg) {
        ...
        async handleMutation(connectionId, msg) {
            const ws = this.connections.getById(connectionId);
            if (!ws)
                return;
            const fn = this.functions[msg.fn];
            ...
            try {
                ...
                ws.send(JSON.stringify({ type: "mutationResult", id: msg.id, result }));
            }
            catch (e) {
        ...
        async handleAction(connectionId, msg) {
            const ws = this.connections.getById(connectionId);
            if (!ws)
                return;
            try {
                const fn = this.functions[msg.fn];
                if (!fn)
                    throw new Error(`Function not found: ${msg.fn}`);
                if (fn.isInternal) {
                    sendError(ws, msg.id, ErrorCode.FORBIDDEN, `Function "${msg.fn}" is internal and cannot be called from a client`);
                    return;
                }
                if (fn.type !== "action")
                    throw new Error(`${msg.fn} is not an action`);
                const actionCtx = this.createActionCtx();
                const result = await fn.handler(actionCtx, msg.args);
                ws.send(JSON.stringify({ type: "actionResult", id: msg.id, result }));
            }
            catch (e) {
                sendError(ws, msg.id, ErrorCode.EXECUTION_ERROR, errorMessage(e));
            }
        ...
        createActionCtx() {
            const storageOps = this.storage.createStorageOps();
            return {
                runQuery: async (fnName, args) => {
                    const fn = this.functions[fnName];
                    if (!fn || fn.type !== "query")
                        throw new Error(`Query not found: ${fnName}`);
                    const txId = crypto.randomUUID();
                    this.transactions.begin(txId, this.latestTs, "query");
                    try {
                ...
                runMutation: async (fnName, args) => {
                    const fn = this.functions[fnName];
                    if (!fn || fn.type !== "mutation")
                        throw new Error(`Mutation not found: ${fnName}`);
                    return executeMutation(this.mutationDeps, fnName, args ?? {});
                },
                runAction: async (fnName, args) => {
                    const fn = this.functions[fnName];
                    if (!fn || fn.type !== "action")
                        throw new Error(`Action not found: ${fnName}`);
                    const actionCtx = this.createActionCtx();
        ...
        // -- Scheduler execution --
        async executeScheduledFunction(fnName, args) {
            const fn = this.functions[fnName];
            if (!fn)
...
function indexRangesToFilter(ranges) {
    if (ranges.length === 0)
        return null;
    const exprs = ranges.map((r) => ({
        op: r.op,
        a: { op: "field", path: r.field },
        b: { op: "literal", value: r.value },
    }));
    if (exprs.length === 1)
        return exprs[0];
    return { op: "and", exprs };
}
//# sourceMappingURL=ZerobackDO.js.map