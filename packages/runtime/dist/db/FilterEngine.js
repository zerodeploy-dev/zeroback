...
function evaluateExpr(row, expr) {
    if (expr.op === "field") {
...
export function applyOrder(rows, field, direction) {
    if (!field) {
        return rows;
    }
...
export function applyLimit(rows, n) {
    if (n === null) {
        return rows;
    }
 ...
 * - Unsafe column names
 */
export function compileFilterToSQL(filter, jsonColumns) {
    try {
...
function compileFilterNode(node, jsonColumns) {
    switch (node.op) {
        case "eq":
        case "neq":
        case "lt":
...
function compileSQLExpr(expr, jsonColumns) {
    if (expr.op === "field") {
