...
import { validatorToSQLType, generateTableDDL, generateSearchIndexDDL, buildTableColumns, docToSQLParams, sqlRowToDoc, migrateSchema, } from "./SchemaMapper";
describe("validatorToSQLType", () => {
    ...
    it("maps object/array/union/record/any to JSON TEXT", () => {
        for (const type of ["object", "array", "union", "record", "any"]) {
            const result = validatorToSQLType({ type, value: {} });
    ...
    it("maps unknown type to JSON TEXT", () => {
        expect(validatorToSQLType({ type: "unknown_type" })).toEqual({
    ...
    it("detects optional boolean as isBoolean", () => {
        const schema = {
            tables: {
                t: {
                    fields: {
                        active: { type: "optional", value: { type: "boolean" } },
                    },
                    indexes: [],
                    searchIndexes: [],
                },
...
describe("docToSQLParams", () => {
    it("serializes document to SQL param array", () => {
            ...
            tables: {
                tasks: {
                    fields: {
                        ...
                        tags: { type: "array", value: { type: "string" } },
                    },
                    indexes: [],
                    searchIndexes: [],
                },
        ...
        const params = docToSQLParams({ _id: "tasks:abc", title: "Test", done: true, tags: ["a", "b"] }, info, 42);
    ...
    it("handles null/undefined as null", () => {
        const schema = {
            tables: {
                t: {
                    fields: { name: { type: "optional", value: { type: "string" } } },
                    indexes: [],
                    searchIndexes: [],
                },
            },
...
describe("sqlRowToDoc", () => {
    it("deserializes SQL row to JS doc", () => {
        ...
        const doc = sqlRowToDoc({ _id: id, _ts: 42, title: "Test", done: 1, tags: '["a","b"]' }, info);
    ...
    it("converts 0 to false for boolean columns", () => {
        ...
        const info = buildTableColumns(schema).get("t");
        const doc = sqlRowToDoc({ _id: "t:01ARZ3NDEKTSV4RRFFQ69G5FAV", _ts: 1, active: 0 }, info);
    ...
    it("omits optional null fields", () => {
        const schema = {
            tables: {
                t: {
                    fields: { bio: { type: "optional", value: { type: "string" } } },
                    indexes: [],
                    searchIndexes: [],
                },
            },
        ...
        const info = buildTableColumns(schema).get("t");
        const doc = sqlRowToDoc({ _id: "t:01ARZ3NDEKTSV4RRFFQ69G5FAV", _ts: 1, bio: null }, info);
    ...
    it("keeps non-nullable null fields as null", () => {
        ...
        const info = buildTableColumns(schema).get("t");
        const doc = sqlRowToDoc({ _id: "t:01ARZ3NDEKTSV4RRFFQ69G5FAV", _ts: 1, name: null }, info);
    ...
    it("excludes _ts from output", () => {
        ...
        const info = buildTableColumns(schema).get("t");
        const doc = sqlRowToDoc({ _id: "t:01ARZ3NDEKTSV4RRFFQ69G5FAV", _ts: 99 }, info);
...
//# sourceMappingURL=SchemaMapper.test.js.map