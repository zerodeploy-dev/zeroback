...
function evaluateExpr<T>(row: T, expr: ExprJSON): unknown {
  if (expr.op === "field") {
    return getByPath(row as Record<string, unknown>, expr.path);
  }
  return expr.value;
}
...
export function applyOrder<T>(
  ...
  return [...rows].sort((a, b) => {
    const aVal = getByPath(a as Record<string, unknown>, field);
    const bVal = getByPath(b as Record<string, unknown>, field);

    let cmp = 0;
    if ((aVal as string | number) < (bVal as string | number)) cmp = -1;
    if ((aVal as string | number) > (bVal as string | number)) cmp = 1;
