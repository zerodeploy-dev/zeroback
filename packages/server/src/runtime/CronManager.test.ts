  ...
  const sql: SqlApi = {
    exec(query: string, ...bindings: unknown[]) {
      ...
      if (q.includes("DELETE FROM cron_jobs WHERE name")) {
        data.cron_jobs = data.cron_jobs.filter((j: any) => j.name !== bindings[0])
        return { toArray: () => [] }
      }
      ...
      if (q.startsWith("SELECT") && q.includes("FROM cron_jobs WHERE name =")) {
        ...
        const found = data.cron_jobs.filter((j: any) => j.name === name)
        return { toArray: () => found }
      }
      if (q.includes("FROM cron_jobs") && q.includes("next_run_at <=")) {
        const now = bindings[0] as number
        return { toArray: () => data.cron_jobs.filter((j: any) => j.next_run_at <= now) }
      }
      if (q.includes("SELECT name FROM cron_jobs")) {
        return { toArray: () => data.cron_jobs.map((j: any) => ({ name: j.name })) }
      }
      ...
      if (q.includes("UPDATE cron_jobs SET fn_name")) {
        ...
        const job = data.cron_jobs.find((j: any) => j.name === name)
        if (job) {
      ...
      if (q.includes("FROM scheduled_jobs") && q.includes("status = 'pending'") && q.includes("run_at <=")) {
        const now = bindings[0] as number
        return { toArray: () => data.scheduled_jobs.filter((j: any) => j.status === "pending" && j.run_at <= now) }
      }
      ...
      if (q.includes("UPDATE scheduled_jobs SET status")) {
        ...
        const id = bindings[0]
        const job = data.scheduled_jobs.find((j: any) => j.id === id)
        if (job) job.status = status
        return { toArray: () => [] }
      }
      if (q.includes("DELETE FROM scheduled_jobs WHERE status IN")) {
        data.scheduled_jobs = data.scheduled_jobs.filter((j: any) => j.status === "pending" || j.status === "running")
        return { toArray: () => [] }
      }
      if (q.includes("DELETE FROM scheduled_jobs WHERE id") && q.includes("status = 'pending'")) {
        data.scheduled_jobs = data.scheduled_jobs.filter((j: any) => !(j.id === bindings[0] && j.status === "pending"))
        return { toArray: () => [] }
      }

      // MIN queries for ensureNextAlarm
      if (q.includes("MIN(run_at)")) {
        const pending = data.scheduled_jobs.filter((j: any) => j.status === "pending")
        const min = pending.length ? Math.min(...pending.map((j: any) => j.run_at)) : null
        return { toArray: () => [{ next: min }] }
      }
      if (q.includes("MIN(next_run_at)")) {
        const min = data.cron_jobs.length ? Math.min(...data.cron_jobs.map((j: any) => j.next_run_at)) : null
        return { toArray: () => [{ next: min }] }
      }
...
describe("CronManager", () => {
  describe("initializeCronJobs()", () => {
    ...
    it("removes cron jobs no longer in code", () => {
      ...
      expect(data.cron_jobs.map((j: any) => j.name)).toEqual(["a"])
    })
  })
  ...
  describe("processAlarm()", () => {
    it("executes due scheduled jobs", async () => {
      ...
      expect(data.scheduled_jobs.filter((j: any) => j.status === "pending")).toHaveLength(0)
